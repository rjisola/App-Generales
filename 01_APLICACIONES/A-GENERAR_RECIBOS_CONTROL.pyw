...
class AntiguedadApp:
    def __init__(self, root):
        ...
        # Separador inicial
        tk.Frame(main_frame, height=1, bg=mgc.COLORS['border']).pack(fill=tk.X, pady=(0, 15))
    ...
    def seleccionar_archivo(self):
        ...
        if self.ruta_archivo_csv:
            ...
            self.lbl_archivo.configure(text=f"✓ {nombre_archivo}", fg=mgc.COLORS['green'])
            mgc.enable_button(self.btn_procesar, 'green')
            self.status_var.set(f"✓ Archivo listo: {nombre_archivo}")
        else:
            self.lbl_archivo.configure(text="Ningún archivo seleccionado", fg=mgc.COLORS['text_secondary'])
            mgc.disable_button(self.btn_procesar)
...
class PlanillaHorasApp:
    ...
    def start_processing(self):
        ...
        # --- Detección de Hoja de Horas ---
        target_sheet_name = None
        try:
            # Abrir solo para leer nombres de hojas (rápido)
            wb_temp = openpyxl.load_workbook(main_path, data_only=True, read_only=True)
            sheet_names = wb_temp.sheetnames
            wb_temp.close()
            
            if "HORAS CONTADOR" in sheet_names:
                target_sheet_name = "HORAS CONTADOR"
            elif "Resumen_Liquidacion" in sheet_names:
                target_sheet_name = "Resumen_Liquidacion"
            else:
                # Si no encuentra ninguna, pedir al usuario que seleccione
                selection_window = tk.Toplevel(self.root)
                selection_window.title("Seleccionar Hoja")
                selection_window.configure(bg=mgc.COLORS['bg_primary'])
                mgc.center_window(selection_window, 400, 350)
                selection_window.transient(self.root)
                selection_window.grab_set()
                
                tk.Label(selection_window, text="No se encontró 'HORAS CONTADOR'.\nSeleccione la hoja correcta:", 
                         font=mgc.FONTS['normal'], bg=mgc.COLORS['bg_primary'], 
                         fg=mgc.COLORS['text_primary'], pady=15).pack()
                
                lb = tk.Listbox(selection_window, font=mgc.FONTS['small'],
                                bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'],
                                selectbackground=mgc.COLORS['blue'], selectforeground='white',
                ...
                def on_select():
                    sel = lb.curselection()
                    if sel:
                        selected_var.set(sheet_names[sel[0]])
                        selection_window.destroy()
                
                mgc.create_button(selection_window, "Confirmar Selección", on_select, color='blue').pack(pady=15, padx=20, fill=tk.X)
                
                self.root.wait_window(selection_window)
                target_sheet_name = selected_var.get()
        ...
        processing_thread = threading.Thread(
            ...
            args=(main_path, ref_path, horas_path, respuesta_bono, bonos, target_sheet_name)
        )
        processing_thread.start()

    def run_process_background(self, main_path, ref_path, horas_path, tiene_bono, bonos, sheet_name):
        try:
            ...
            if sheet_name not in wb.sheetnames:
                raise ValueError(f"No se encontro la hoja '{sheet_name}' en el archivo principal.")
            ws = wb[sheet_name]
...
class BonoDialog(simpledialog.Dialog):
    def body(self, master):
        ...
        master.configure(bg=mgc.COLORS['bg_primary'])
        tk.Label(master, text="Elija cómo se debe liquidar el bono:", 
                 font=mgc.FONTS['normal'], bg=mgc.COLORS['bg_primary'], 
                 fg=mgc.COLORS['text_primary'], justify=tk.LEFT).pack(pady=15, padx=20)
        ...
        for texto, valor in opciones:
            rb = tk.Radiobutton(master, text=texto, variable=self.v, value=valor,
                                font=mgc.FONTS['small'], bg=mgc.COLORS['bg_primary'],
                                fg=mgc.COLORS['text_primary'], selectcolor=mgc.COLORS['bg_input'],
                                activebackground=mgc.COLORS['bg_primary'], 
                                activeforeground=mgc.COLORS['text_primary'])
...
class SheetSelectDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        master.configure(bg=mgc.COLORS['bg_primary'])
        tk.Label(master, text="No se encontró la hoja 'HORAS CONTADOR'.\nPor favor, elija la hoja que contiene los datos de horas:", 
                 font=mgc.FONTS['normal'], bg=mgc.COLORS['bg_primary'], 
                 fg=mgc.COLORS['text_primary'], justify=tk.LEFT).pack(pady=15, padx=20)
        
        self.listbox = tk.Listbox(master, font=mgc.FONTS['small'],
                                  bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'],
                                  selectbackground=mgc.COLORS['blue'], selectforeground='white',
                                  borderwidth=1, relief=tk.FLAT, exportselection=False, height=8)
...
def crear_ventana_antiguedad(root: tk.Tk) -> tk.Toplevel:
    ...
    mgc.center_window(ventana, 900, 700)
    return ventana
...
def crear_ventana_recibos(root: tk.Tk, search_path: str) -> tk.Toplevel:
    ...
    mgc.center_window(ventana, 1050, 700)
    set_window_icon(ventana, 'receipts')
    ventana.icon_receipts = load_icon('receipts', (64, 64))
    ...
    # Frame principal con scroll (estandarizado)
    main_frame = mgc.create_main_container(ventana)
    
    # Header moderno con descripción
    mgc.create_header(main_frame, "Generar Recibos", 
                     "Genera un recibo de sueldo completo en PDF para cada empleado, tomando los datos directamente del archivo de nómina procesado.",
                     icon_image=ventana.icon_receipts)
    tk.Frame(main_frame, height=1, bg=mgc.COLORS['border']).pack(fill=tk.X, pady=(0, 15))
    ...
    tk.Entry(emp_frame, textvariable=cant_empleados_var, font=mgc.FONTS['normal'], 
             width=8, bg=mgc.COLORS['bg_input'], fg=mgc.COLORS['text_primary'], 
             insertbackground=mgc.COLORS['text_primary']).pack(side=tk.LEFT, padx=10)
    
    # Bono
    tk.Checkbutton(card3_inner, text="Liquidar bono", variable=agregar_bono_var,
                   font=mgc.FONTS['normal'], bg=mgc.COLORS['bg_card'],
                   fg=mgc.COLORS['text_primary'], activebackground=mgc.COLORS['bg_card'],
                   activeforeground=mgc.COLORS['text_primary'],
                   selectcolor=mgc.COLORS['bg_input']).pack(anchor='w')
    ...
    tk.Radiobutton(bono_opts, text="No remun.", variable=tipo_bono_var, value='no_remunerativo',
                   bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'],
                   activebackground=mgc.COLORS['bg_card'], activeforeground=mgc.COLORS['text_primary'],
                   selectcolor=mgc.COLORS['bg_input']).pack(anchor='w')
    ...
    tk.Label(f_row, text="Feriados:", bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'], 
             width=10, anchor='w').pack(side=tk.LEFT)
    tk.Entry(f_row, textvariable=cant_feriados_var, width=5, 
             bg=mgc.COLORS['bg_input'], fg=mgc.COLORS['text_primary'], 
             insertbackground=mgc.COLORS['text_primary']).pack(side=tk.LEFT)
    
    a_row = tk.Frame(uecara_opts, bg=mgc.COLORS['bg_card'])
    a_row.pack(fill=tk.X)
    tk.Label(a_row, text="Ausencias:", bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'], 
             width=10, anchor='w').pack(side=tk.LEFT)
    tk.Entry(a_row, textvariable=cant_ausencias_var, width=5, 
             bg=mgc.COLORS['bg_input'], fg=mgc.COLORS['text_primary'], 
             insertbackground=mgc.COLORS['text_primary']).pack(side=tk.LEFT)
    ...
    tk.Checkbutton(card2_inner, text="Generar PDFs", variable=generar_pdfs_var,
                   font=mgc.FONTS['normal'], bg=mgc.COLORS['bg_card'],
                   fg=mgc.COLORS['text_primary'], activebackground=mgc.COLORS['bg_card'],
                   activeforeground=mgc.COLORS['text_primary'], selectcolor=mgc.COLORS['bg_input']).pack(anchor='w')
    ...
    tk.Checkbutton(card_extras_inner, text="Ordenar por Índice", variable=usar_indice_var,
                   bg=mgc.COLORS['bg_card'], fg=mgc.COLORS['text_primary'],
                   activebackground=mgc.COLORS['bg_card'], activeforeground=mgc.COLORS['text_primary'],
                   selectcolor=mgc.COLORS['bg_input']).pack(anchor='w', pady=(5,0))
...
# ==============================================================================
# ==============================================================================
# TOOLTIP FLOTANTE PARA CARDS DEL ASISTENTE (igual al launcher principal)
# ==============================================================================
class _ToolTipCard:
    """Muestra un tooltip flotante sobre cualquier widget de Tkinter."""
    ...
    def show_tip(self, event=None):
        ...
        frame = tk.Frame(tw, bg='#1e293b', padx=14, pady=10, bd=0)
        frame.pack()

        tk.Label(frame, text=self.title,
                 font=('Segoe UI', 13, 'bold'),
                 bg='#1e293b', fg='#93c5fd').pack(anchor='center')

        tk.Label(frame, text=self.text,
                 font=('Segoe UI', 11),
                 bg='#1e293b', fg='#e2e8f0',
                 wraplength=320, justify='center').pack(anchor='center', pady=(4, 0))
...
class AsistenteUnificadoApp:
    def __init__(self, root: tk.Tk) -> None: 
        ...
        # Cargar iconos
        set_window_icon(self.root, 'main')
        self.icon_main = load_icon('main', (64, 64))

        # Cargar iconos para las cards igual que el Launcher principal
        self.icon_calendar = load_icon('calendar', (48, 48))
        self.icon_chart = load_icon('chart', (48, 48))
        self.icon_receipts = load_icon('receipts', (48, 48))
    ...
    def _make_tool_card(self, parent, icon_img, title, description, action, color):
        """Crea una card completamente clickeable, con hover y tooltip, compatible con CustomTkinter."""
        card_outer, card_inner = mgc.create_card(parent, padding=18)
        
        # Icono PNG premium
        icon_lbl = ctk.CTkLabel(card_inner, image=icon_img, text="", fg_color="transparent")
        icon_lbl.pack(pady=(0, 6))

        # Nombre / título
        name_lbl = ctk.CTkLabel(card_inner, text=title,
                                font=('Segoe UI', 14, 'bold'),
                                text_color=mgc.COLORS[color],
                                wraplength=280, justify='center')
        name_lbl.pack(pady=(0, 4))

        # Descripción
        desc_lbl = ctk.CTkLabel(card_inner, text=description,
                                font=mgc.FONTS['small'],
                                text_color=mgc.COLORS['text_secondary'],
                                wraplength=280, justify='center')
        desc_lbl.pack(pady=(0, 6))

        # Indicador "Haga clic para abrir"
        hint_lbl = ctk.CTkLabel(card_inner, text="▶  Haga clic para abrir",
                                font=('Segoe UI', 8),
                                text_color=mgc.COLORS['text_secondary'])
        hint_lbl.pack()

        # Todos los widgets que participan en el hover y click
        interactive = [card_outer, card_inner, icon_lbl, name_lbl, desc_lbl, hint_lbl]

        # Aplicar cursor mano a todos
        ...
        # Click → ejecutar acción
        for w in interactive:
            w.bind('<Button-1>', lambda e, fn=action: fn())

        # El widget 'card' real es el hijo de card_outer en el componente de mgc
        # Buscamos el frame que tiene el borde
        card_real = card_outer.winfo_children()[0] if card_outer.winfo_children() else card_outer

        # Hover — resaltado visual compatible con CTk
        def on_enter(e):
            try:
                card_real.configure(border_color=mgc.COLORS[color], border_width=2)
            except:
                pass
            
            hover_bg = '#f8fafc' if mgc._APPEARANCE_MODE == "Light" else '#1e293b'
            card_real.configure(fg_color=hover_bg)
            card_inner.configure(fg_color="transparent")
            
            hint_lbl.configure(text_color=mgc.COLORS[color])
            try:
                card_outer.grid_configure(pady=(2, 8))
            except: pass

        def on_leave(e):
            try:
                card_real.configure(border_color=mgc.COLORS['border'], border_width=1)
            except:
                pass
                
            normal_bg = mgc.COLORS['bg_card']
            card_real.configure(fg_color=normal_bg)
            
            hint_lbl.configure(text_color=mgc.COLORS['text_secondary'])
            try:
                card_outer.grid_configure(pady=5)
            except: pass
        ...
        # Tooltip flotante
        _ToolTipCard(card_outer, title, description)
    ...
    def create_widgets(self, parent: tk.Widget) -> None: 
        ...
        mgc.create_header(parent,
                         ...
                         "Herramientas avanzadas para el procesamiento de recibos",
                         icon_image=self.icon_main)
        
        # Separador inicial
        tk.Frame(parent, height=1, bg=mgc.COLORS['border']).pack(fill=tk.X, pady=(0, 20))
        
        # Grid de herramientas — 2 columnas iguales
        ...
        # Card 1: Antigüedad
        card1 = self._make_tool_card(
            tools_frame,
            icon_img=self.icon_calendar,
            title="Antigüedad",
            description="Calcula años de antigüedad desde fechas de ingreso",
            action=self.run_step1,
            color='purple'
        )
        card1.grid(row=0, column=0, sticky='nsew', padx=5, pady=5)

        # Card 2: Planilla de Horas
        card2 = self._make_tool_card(
            tools_frame,
            icon_img=self.icon_chart,
            title="Planilla de Horas",
            description="Procesa y calcula valores de horas trabajadas",
            action=self.run_step2,
            color='blue'
        )
        card2.grid(row=0, column=1, sticky='nsew', padx=5, pady=5)

        # Card 3: Generar Recibos (ocupa toda la fila inferior)
        card3 = self._make_tool_card(
            tools_frame,
            icon_img=self.icon_receipts,
            title="Generar Recibos",
            description="Genera recibos de sueldo en Excel y PDF",
            action=self.run_step3,
            color='green'
        )
        card3.grid(row=1, column=0, columnspan=2, sticky='nsew', padx=5, pady=5)
        
        # Separador
        tk.Frame(parent, height=1, bg=mgc.COLORS['border']).pack(fill=tk.X, pady=15)
    ...
    def _configure_child_window(self, window: tk.Toplevel) -> None: 
        ...
        # Mantener siempre por encima de la principal y hacerla modal
        ...
        _ubicar_en_cascada(window, self.root)
    ...
    def _open_tool_window(
        ...
        try:
            ...
            messagebox.showerror("Error al abrir herramienta",
...
if __name__ == "__main__":
    root = ctk.CTk()
    root.withdraw() # Ocultar la raiz ya que usamos Toplevel
    search_path = os.path.dirname(os.path.abspath(__file__))
    
    # Crear y mostrar unicamente la ventana de recibos
    ventana = crear_ventana_recibos(root, search_path)
    
    # Asegurar que al cerrar la ventana de recibos, se cierre todo el programa
