...
# Importar componentes modernos del proyecto
try:
...
def procesar_guia_pago(archivo_origen: str, banco_filtro: str,
    ...
    log(f"📂 Abriendo archivo origen...")
    try:
        wb_origen = openpyxl.load_workbook(archivo_origen, data_only=True, keep_vba=True)
    ...
    # --- Abrir destino ---
    log(f"📂 Abriendo archivo destino...")
    try:
...
class GuiaPagoBancarioApp:
    ...
    def _build_ui(self):
        ...
        # ── HEADER ────────────────────────────────────────────────────────────        # Header moderno (estilo launcher)
        mgc.create_header(main, "Guía Pago Bancario", 
                         "Filtrar por banco y actualizar archivos de Guía de Pago", 
                         icon_image=load_icon('payroll', (64, 64)) if HAS_ICON else None)
        
        # Separador inicial
        tk.Frame(main, height=1, bg=mgc.COLORS['border']).pack(fill=tk.X, pady=(0, 15))
        ...
        self.btn_procesar.bind("<Leave>", lambda e: self.btn_procesar.config(bg=COLORES['green']))
    ...
    def _on_banco_change(self):
        ...
        if banco == "SANTANDER":
            self.btn_snt.config(relief=tk.SUNKEN, bg='#3b0a0a')
            self.btn_otros.config(relief=tk.RAISED, bg=COLORES['bg_card'])
        else:
            self.btn_otros.config(relief=tk.SUNKEN, bg='#0a1a3b')
            self.btn_snt.config(relief=tk.RAISED, bg=COLORES['bg_card'])

        # Actualizar label destino
        ruta = ARCHIVOS_DESTINO.get(banco, "")
        if hasattr(self, 'lbl_destino_val'):
            self.lbl_destino_val.config(text=ruta)
