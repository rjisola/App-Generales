...
class UnifiedCalculatorApp:
    ...
    def setup_uocra_tab(self):
        ...
        tk.Checkbutton(f_grid, text="Seguro de Vida", variable=self.var_seguro_uocra, 
                      onvalue="Seg Vida", offvalue="Sin Seg Vida", 
                      bg=mgc.COLORS['bg_card'], font=('Segoe UI', 9), 
                      activebackground=mgc.COLORS['bg_card']).grid(row=11, column=2, columnspan=2, sticky='w', padx=3, pady=2)
        ...
        tk.Label(f_grid, text="Tipo Bono:", bg=mgc.COLORS['bg_card'], font=('Segoe UI', 9)).grid(row=13, column=0, sticky='w', padx=3, pady=2)
        ...
        btn_calc = mgc.create_large_button(btn_frame, "CALCULAR", self.calculate_uocra, color='green', icon_image=self.icon_check)
        btn_calc.pack()
        btn_calc.bind("<Enter>", lambda e: btn_calc.config(bg='#059669'))
        btn_calc.bind("<Leave>", lambda e: btn_calc.config(bg=mgc.COLORS['green']))
    ...
    def setup_uecara_tab(self):
        ...
        # Retención Judicial
        lbl_judicial = tk.Label(f_grid, text="⚖️ Retención Judicial", bg=mgc.COLORS['bg_card'], 
                               font=('Segoe UI', 8, 'bold'), fg=mgc.COLORS['red'])
        ...
        btn_calc.pack()
        btn_calc.bind("<Enter>", lambda e: btn_calc.config(bg='#7c3aed'))
        btn_calc.bind("<Leave>", lambda e: btn_calc.config(bg=mgc.COLORS['purple']))
