...
class AguinaldoUnificadoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🎯 Asistente de Aguinaldo UNIFICADO")
    ...
    def setup_ui(self):
        ...
        # Título modernizado (estilo launcher)
        tk.Label(sidebar, text="🎯 AGUINALDO", bg=self.colors['card'], fg=self.colors['text'], 
                font=("Segoe UI", 18, "bold"), justify='center').pack(pady=(30, 5))
        tk.Label(sidebar, text="UNIFICADO", bg=self.colors['card'], fg=self.colors['accent'], 
        ...
        self._create_sidebar_label(sidebar, "   (Resta J/L del Total H)", is_info=True)
        
        self._create_dark_radio(sidebar, "💰 Negro (Bruto/Neto)", 2, self.on_tipo_changed)
        self._create_sidebar_label(sidebar, "   (Hojas 1ªQ 01 2026)", is_info=True)
        
        self._create_dark_radio(sidebar, "💸 Efectivo (Columna G)", 3, self.on_tipo_changed)
        self._create_sidebar_label(sidebar, "   (Total Col G del Recuento)", is_info=True)
        
        self._create_dark_radio(sidebar, "📋 Sueldo/Efectivo (Archivo Único)", 4, self.on_tipo_changed)
        self._create_sidebar_label(sidebar, "   (Para archivo Informe RIM)", is_info=True)
        ...
        tk.Spinbox(sidebar, from_=2015, to=2035, textvariable=self.year_var, bg=self.colors['input'], 
                  fg=self.colors['success'], buttonbackground=self.colors['card'], relief='solid', bd=1, 
                  font=("Segoe UI", 11, "bold")).pack(fill='x', padx=20, pady=(0, 10), ipady=5)
        ...
        # --- MAIN PANEL CON SCROLL ---
        self.main_container = mgc.create_main_container(self.root, padding=0)
        self.main_panel = tk.Frame(self.main_container, bg=self.colors['bg'])
        self.main_panel.pack(fill='both', expand=True, padx=20, pady=20)

        # Header modernizado en el panel principal
        mgc.create_header(self.main_panel, "Configuración de Archivos", 
                         "Defina las rutas de los archivos para el cálculo del aguinaldo", 
                         icon_image=load_icon('bonus_white', (64, 64)))
        
        # Separador inicial
        tk.Frame(self.main_panel, height=1, bg=self.colors['border']).pack(fill='x', pady=(0, 20))
        ...
        idx_row.pack(fill='x')
        tk.Entry(idx_row, textvariable=self.indice_path, bg=self.colors['input'], fg=self.colors['success'], 
                relief='flat', font=("Segoe UI", 10, "bold")).pack(side='left', fill='x', expand=True, ipady=8, padx=(0, 10))
    ...
    def _create_sidebar_label(self, parent, text, is_info=False):
        color = self.colors['accent'] if is_info else self.colors['text_dim']
        font_size = 8 if is_info else 8
        tk.Label(parent, text=text, bg=self.colors['card'], fg=color, 
                font=("Segoe UI", font_size, "bold"), anchor='w').pack(fill='x', padx=20, pady=(0, 5))
    ...
    def _create_dark_radio(self, parent, text, value, command=None, variable=None):
        ...
        tk.Radiobutton(parent, text=text, variable=var, value=value, bg=self.colors['card'], 
                      fg=self.colors['text'], selectcolor=self.colors['card'], activebackground=self.colors['card'], 
    ...
    def update_dynamic_area(self):
        """Actualiza el estado de los componentes según el tipo de procesamiento"""
        # Limpiar área dinámica solo la primera vez para crear la estructura fija
        if not hasattr(self, 'folder_frame'):
            for widget in self.dynamic_area.winfo_children():
                widget.destroy()
            self._create_unified_ui_structure()

        tipo = self.tipo_proceso.get()
        
        if tipo in [1, 2, 3]:  # Búsqueda Automática (Carpeta)
            self._set_frame_state(self.folder_frame, "normal")
            self._set_frame_state(self.file_frame, "disabled")
            self.folder_status_label.configure(text="MODO CARPETA ACTIVO", fg=self.colors['success'])
            self.file_status_label.configure(text="(No necesario para este modo)", fg=self.colors['text_dim'])
        else:  # Archivo Único
            self._set_frame_state(self.folder_frame, "disabled")
            self._set_frame_state(self.file_frame, "normal")
            self.folder_status_label.configure(text="(No necesario para este modo)", fg=self.colors['text_dim'])
            self.file_status_label.configure(text="MODO ARCHIVO ÚNICO ACTIVO", fg=self.colors['success'])

    def _set_frame_state(self, frame, state):
        """Recursivamente activa o desactiva widgets en un frame"""
        for child in frame.winfo_children():
            try:
                if isinstance(child, (tk.Button, tk.Entry, tk.Checkbutton, tk.Radiobutton, tk.Spinbox)):
                    child.configure(state=state)
                    # Ajustar color de fondo para botones
                    if isinstance(child, tk.Button):
                        if state == "disabled":
                            child.configure(bg=self.colors['input'])
                        else:
                            # Restaurar color original según el texto o contexto
                            if "CARPETA" in child.cget("text") or "BUSCAR" in child.cget("text") or "Examinar" in child.cget("text"):
                                child.configure(bg=self.colors['accent'])
                            elif "+" in child.cget("text"):
                                child.configure(bg=self.colors['success'])
                            elif "-" in child.cget("text"):
                                child.configure(bg=self.colors['danger'])
                elif isinstance(child, (tk.Frame, tk.LabelFrame)):
                    self._set_frame_state(child, state)
            except:
                pass

    def _create_unified_ui_structure(self):
        """Crea la estructura fija de la UI con ambas secciones"""
        
        # --- SECCIÓN 1: CARPETA DE BÚSQUEDA ---
        self.folder_frame = tk.LabelFrame(self.dynamic_area, text="📂 BÚSQUEDA EN CARPETA (MODO AUTOMÁTICO)", 
                                         bg=self.colors['bg'], fg=self.colors['text'], font=("Segoe UI", 10, "bold"),
                                         padx=15, pady=15, relief='flat', highlightthickness=1, highlightbackground=self.colors['border'])
        self.folder_frame.pack(fill='x', pady=(0, 20))
        
        self.folder_status_label = tk.Label(self.folder_frame, text="", bg=self.colors['bg'], font=("Segoe UI", 8, "italic"))
        self.folder_status_label.pack(anchor='e')

        search_row = tk.Frame(self.folder_frame, bg=self.colors['bg'])
        search_row.pack(fill='x', pady=10)
        tk.Entry(search_row, textvariable=self.carpeta_base, bg=self.colors['input'], fg=self.colors['success'], 
                relief='flat', font=("Segoe UI", 10, "bold")).pack(side='left', fill='x', expand=True, ipady=8, padx=(0, 10))
        self._create_tool_btn(search_row, "📂 CARPETA", self.select_carpeta, self.colors['accent'])
        self._create_tool_btn(search_row, "🔍 BUSCAR", self.buscar_archivos, self.colors['accent'])

        # Resultados de búsqueda
        tree_container = tk.Frame(self.folder_frame, bg=self.colors['input'])
        tree_container.pack(fill='x', pady=5)
        
        cols = ('mes', 'quincena', 'archivo', 'estado')
        self.tree = ttk.Treeview(tree_container, columns=cols, show='headings', height=4)
        for col in cols: self.tree.heading(col, text=col.capitalize())
        self.tree.column('mes', width=80); self.tree.column('quincena', width=70)
        self.tree.column('archivo', width=300); self.tree.column('estado', width=80)
        self.tree.pack(side='left', fill='x', expand=True)
        
        ctrl_row = tk.Frame(self.folder_frame, bg=self.colors['bg'])
        ctrl_row.pack(fill='x', pady=(5, 0))
        self._create_tool_btn(ctrl_row, "+ MANUAL", self.agregar_archivo_manual, self.colors['success'])
        self._create_tool_btn(ctrl_row, "- QUITAR", self.quitar_archivo_manual, self.colors['danger'])
        self.resumen_label = tk.Label(ctrl_row, text="Esperando...", bg=self.colors['bg'], fg=self.colors['text_dim'])
        self.resumen_label.pack(side='right')

        # --- SECCIÓN 2: ARCHIVO ÚNICO ---
        self.file_frame = tk.LabelFrame(self.dynamic_area, text="📋 ARCHIVO CONSOLIDADO (MODO ÚNICO)", 
                                       bg=self.colors['bg'], fg=self.colors['text'], font=("Segoe UI", 10, "bold"),
                                       padx=15, pady=15, relief='flat', highlightthickness=1, highlightbackground=self.colors['border'])
        self.file_frame.pack(fill='x')

        self.file_status_label = tk.Label(self.file_frame, text="", bg=self.colors['bg'], font=("Segoe UI", 8, "italic"))
        self.file_status_label.pack(anchor='e')

        file_row = tk.Frame(self.file_frame, bg=self.colors['bg'])
        file_row.pack(fill='x', pady=10)
        tk.Entry(file_row, textvariable=self.quincenas_path, bg=self.colors['input'], fg=self.colors['success'], 
                relief='flat', font=("Segoe UI", 10, "bold")).pack(side='left', fill='x', expand=True, ipady=8, padx=(0, 10))
        self._create_tool_btn(file_row, "📂 Examinar", self.select_quincenas, self.colors['accent'])

        # Info pequeña
        tk.Label(self.file_frame, text="ℹ️ Ideal para Informe RIM (Col E y J)", bg=self.colors['bg'], 
                fg=self.colors['text_dim'], font=("Segoe UI", 8, "italic")).pack(anchor='w')
    ...
    def buscar_archivos(self):
        ...
        patron = re.compile(r'(?:PROGRAMA\s+DEPOSITO\s+)?(1ERA|2DA|1RA)\s+(\w+)\s*(\d{4})', re.IGNORECASE)
    ...
    def _search_thread(self, patron, meses_buscar, year):
        try:
            ...
            for archivo in archivos_excel:
                ...
                if match:
                    quincena = match.group(1).upper().replace('1RA', '1ERA')
    ...
    def process_blanco(self):
        ...
        try:
            ...
            filename = f"Resumen_Aguinaldo_BLANCO_{semestre_nombre}_{current_year}.xlsx"
            output_filename = os.path.join(os.path.dirname(script_dir), filename)
    ...
    def process_negro(self):
        ...
        try:
                ...
                if mes_nombre in self.archivos_encontrados:
                    for quincena_nombre, filepath in self.archivos_encontrados[mes_nombre].items():
                        ...
                        try:
                            ...
                            # Fallback: Si no encuentra hoja por nombre de quincena, buscar RECUENTO TOTAL
                            if not s_name:
                                s_name = self.find_target_sheet_in_file(filepath)
            ...
            for f_path, sheet, quincena_num in sheets_to_process:
                ...
                try:
                    df_sheet = pd.read_excel(f_path, sheet_name=sheet, header=None)
                    
                    if df_sheet.shape[1] >= 8:
                        df_q = df_sheet.iloc[:, [0, 5, 6, 7]].copy()
                        df_q.columns = ['legajo', 'Neto_F', 'Neto_G', 'Neto_H']
                    elif df_sheet.shape[1] >= 7:
                        df_q = df_sheet.iloc[:, [0, 5, 6]].copy()
                        df_q.columns = ['legajo', 'Neto_F', 'Neto_G']
                        df_q['Neto_H'] = 0.0
                    else:
                        self.warnings.append(f"Advertencia: No hay suficientes columnas en la hoja '{sheet}'.")
                        self.progress_bar.step()
                        continue
                    ...
                    neto_f = pd.to_numeric(df_q['Neto_F'], errors='coerce').fillna(0)
                    neto_g = pd.to_numeric(df_q['Neto_G'], errors='coerce').fillna(0)
                    neto_h = pd.to_numeric(df_q['Neto_H'], errors='coerce').fillna(0)
                    
                    temp_neto = neto_f.where(neto_f > 0, neto_g)
                    df_q['Neto'] = temp_neto.where(temp_neto > 0, neto_h)
                    df_q['Bruto'] = df_q['Neto']  # Duplicar el Neto de RECUENTO TOTAL (2) en Bruto

                    df_quincena = df_q.groupby('legajo', as_index=False)[['Bruto', 'Neto']].sum()
            ...
            filename = f"Resumen_Aguinaldo_NEGRO_{semestre_nombre}_{current_year}.xlsx"
            output_filename = os.path.join(os.path.dirname(script_dir), filename)
    ...
    def process_efectivo(self):
        ...
        try:
            ...
            filename = f"Resumen_Aguinaldo_EFECTIVO_{semestre_nombre}_{current_year}.xlsx"
            output_filename = os.path.join(os.path.dirname(script_dir), filename)
    ...
    def process_sueldo_efectivo(self):
        ...
        try:
            ...
            filename = f"Resumen_Aguinaldo_{semestre_nombre}_{current_year}.xlsx"
            output_filename = os.path.join(os.path.dirname(script_dir), filename)
    ...
    def find_sheet(self, available_sheets, q_num, month, year):
        ...
        patterns = [
            # Formatos ultra-flexibles para capturar "1ªQ 012026", "1 Q 01 2026", etc.
            rf"^{q_num}.*Q.*{month_str}.*{year}$",
            rf"^{q_num}.*Q.*{month_str}{year}$",
            rf"{q_num}.*Q.*{month_str}.*{year}",
            rf"{q_num}.*Q.*{month_str}{year}",
        ]
...
if __name__ == "__main__":
    try:
        ...
        try:
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, f"Error fatal en Asistente Aguinaldo:\n{str(e)}", "Error de Inicio", 0x10)
        except:
            pass
