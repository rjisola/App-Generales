Sub completarImprimirMarron(fila, contador, columna, desplazamiento, color)
    ...
    End If
    'Total quincena
    totalQuincena = totalExtras + premio + reintegro + importeHorasAltura + sueldoSobre + adelanto + ajusteAlquiler
    ...
    Hoja3.Cells(contador + 5, columna).Value = "TOTAL EXTRAS"
    Hoja3.Cells(contador + 5, columna + 1).Value = totalExtras
    ...
    If reintegro <> 0 Then
        Hoja3.Cells(contador + 6, columna).Value = "REINTEGRO"
        Hoja3.Cells(contador + 6, columna + 1).Value = reintegro
    End If
