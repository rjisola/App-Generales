Sub completarImprimirVerde(fila, contador, columna, desplazamiento, color)

    'NO TIENE MAS HORAS EXTRAS NI FERIADOS (CAMBIO REALIZADO 15-3-19)
    ...
    'Total quincena
    totalQuincena = sueldoAcordado + importeHorasAlCien + ajusteAlquiler + varios + reintegro + adelanto
    ...
    premio = sueldoAcordado - importeHorasNormales
    'horas normales
    ...
    Hoja1.Cells(contador + 4, columna + 1).Value = horasNormales + horasAlCien
    Hoja1.Cells(contador + 4, columna + 2).Value = importeHorasNormales
    
    'Horas al cien
    Call unMergearCeldas(contador + 5, coluna + 1, columna + 2, Hoja1)
    ...
    If ajusteAlquiler <> 0 Then
        Hoja1.Cells(contador + 8, columna).Value = "ALQUILER"
        Hoja1.Cells(contador + 8, columna + 1).Value = ajusteAlquiler
    End If
    ...
    Hoja10.Cells(fila - 7, 1 + 2).Interior.color = color
    Hoja10.Cells(fila - 7, 5 + 2).Value = totalQuincena
    Hoja10.Cells(fila - 7, 1 + 2).Value = nombre
    ...
    If IsEmpty(Hoja4.Cells(fila, 3)) And IsEmpty(Hoja4.Cells(fila, 4)) Then
        Hoja10.Cells(fila - 7, 4 + 2).Value = banco + cajaDeAhorro
    Else
        If IsEmpty(Hoja4.Cells(fila, 3)) And Not IsEmpty(Hoja4.Cells(fila, 4)) Then
            Hoja10.Cells(fila - 7, 4 + 2).Value = banco
        Else
            Hoja10.Cells(fila - 7, 2 + 2).Value = banco
        End If
        If IsEmpty(Hoja4.Cells(fila, 4)) And Not IsEmpty(Hoja4.Cells(fila, 3)) Then
            Hoja10.Cells(fila - 7, 4 + 2).Value = cajaDeAhorro
        Else
            Hoja10.Cells(fila - 7, 3 + 2).Value = cajaDeAhorro
        End If
    End If
