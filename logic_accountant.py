...
def process_accountant_summary_for_employee(employee_data, config, day_definitions, rate_config, wb_styles=None, row_idx=None, sheet_name='CALCULAR HORAS'):
    ...
    categoria_empleado = str(employee_data.get('CATEGORÍA', '')).strip()
    
    # IMPORTANTE: Empleados sin categoría (jubilados) se consideran ESPECIALIZADO
    # para la planilla de envío al contador
    ...
    # Si la categoría no existe en job_title_rates, usar ESPECIALIZADO como fallback
    # Esto aplica a categorías como ANALISTA TECNICO, CAPATAZ OBRA, ADMINISTRACION, etc.
    if not cat_info:
    ...
    for day_info in day_definitions:
        ...
        if ws_styles and row_idx and col_idx:
            ...
            if cell.fill and cell.fill.fgColor:
                ...
                if is_debug_target and day_input_raw and str(day_input_raw) != '0' and str(day_input_raw) != 'nan':
                     print(f"  Day {day_name} (Col {col_idx}): Val={day_input_raw}, RGB={rgb}")

                if rgb:
                    if matches_color(rgb, RGB_NARANJA) or matches_color(rgb, RGB_NARANJA_CLARO):
                        day_color_type = 'NARANJA'
                        if is_debug_target: print(f"    -> Detectado NARANJA")
                    elif matches_color(rgb, RGB_VERDE) or matches_color(rgb, RGB_MARRON):
                        day_color_type = 'PAPELERA'
                        if is_debug_target: print(f"    -> Detectado PAPELERA")
                    # Si no coincide con ninguno, se queda en BLANCO (por defecto)
        ...
        try:
            ...
            if val > 0 and day_name not in ['sábado', 'domingo']:
                ...
                if day_color_type == 'BLANCO': horas_blanco += horas_a_sumar
                    ...
                    if is_debug_target: print(f"    -> Sumando {horas_a_sumar} a Quilmes. Total: {horas_quilmes}")
