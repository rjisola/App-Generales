...
_excel_data_cache = {}
_CUSTOM_EXCEL_PATH = None

def set_custom_excel_path(path):
    """Permite establecer una ruta personalizada para el Excel de tarifas."""
    global _CUSTOM_EXCEL_PATH, _excel_data_cache
    if path and os.path.exists(path):
        _CUSTOM_EXCEL_PATH = path
        _excel_data_cache = {} # Limpiar caché para forzar recarga
        return True
    return False
...
def _load_excel_data():
    ...
    if _CUSTOM_EXCEL_PATH:
        target_path = _CUSTOM_EXCEL_PATH
    else:
        # Fallback a la lógica original si no hay ruta personalizada
        if os.path.exists(_EXCEL_RESPALDO):
            target_path = _EXCEL_RESPALDO
        elif os.path.exists(_EXCEL_PRIMARIO):
            target_path = _EXCEL_PRIMARIO
        else:
            target_path = _EXCEL_RESPALDO
