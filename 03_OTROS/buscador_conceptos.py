...
def extract_employee_data(text):
    ...
    for line in lines:
        ...
        if match:
             return match.group(1), format_nombre_propio(match.group(2)), match.group(3)
        ...
        if not legajo:
            ...
            if match_simple:
                ...
                nombre = format_nombre_propio(match_simple.group(2))
...
def search_in_pdfs_pivot(pdf_paths, concept, indice_path=None, fecha_tope="", extract_units=False, callback=None, filtro_firma="empleador"):
...
def search_in_pdfs(pdf_paths, concept, callback=None, filtro_firma="empleador"):
    results = search_in_pdfs_pivot(pdf_paths, concept, callback=callback, filtro_firma=filtro_firma)
