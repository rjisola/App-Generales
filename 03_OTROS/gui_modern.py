...
class PayrollModernGUI:
    ...
    def create_widgets(self):
        ...
        self.header = self.mgc.create_header(
            self.scroll_container, 
            "Procesar Nómina", 
            "Gestión automatizada de sueldos, horas y resúmenes contables", 
            icon_image=load_icon('payroll', (64, 64))
        )
        self.header.pack(fill='x', padx=30, pady=(30, 10))

        # Frame de contenido principal (dentro del scroll)
        ...
        grid_frame.columnconfigure(0, weight=5)
        grid_frame.columnconfigure(1, weight=3)
        
        self.create_file_card(grid_frame)
        self.create_progress_card(grid_frame)
        
        # Panel inferior de doble columna
        bottom_grid = ctk.CTkFrame(content_frame, fg_color="transparent")
        bottom_grid.pack(fill='both', expand=True, pady=(15, 0))
        bottom_grid.columnconfigure(0, weight=5)
        bottom_grid.columnconfigure(1, weight=3)

        self.create_left_panel(bottom_grid)
        self.create_log_card(bottom_grid)

        # Botones de acción inferiores fijos a la ventana (fuera del scroll)
        self.create_action_buttons(self.root)
        
    def create_file_card(self, parent):
        card_outer, inner = self.mgc.create_card(parent, "📂 ARCHIVO DE ENTRADA")
        card_outer.grid(row=0, column=0, sticky='nsew', padx=(0, 10))
        
        self.file_label = ctk.CTkLabel(inner, text="Ninguno seleccionado", font=self.mgc.FONTS['normal'], text_color=self.colors['text_secondary'])
        self.file_label.pack(pady=(5, 12))
        
        browse_btn = self.mgc.create_button(inner, "Seleccionar Archivo .XLSM", self.browse_file, color='purple', icon="📂")
        browse_btn.pack(fill='x', padx=20)
        
    def create_progress_card(self, parent):
        card_outer, inner = self.mgc.create_card(parent, "📊 ESTADO Y PROGRESO")
        card_outer.grid(row=0, column=1, sticky='nsew')
        
        self.progress_bar = ctk.CTkProgressBar(inner, height=12, corner_radius=6, progress_color=self.colors['blue'])
        self.progress_bar.set(0)
        self.progress_bar.pack(fill='x', padx=20, pady=(12, 8))
        
        self.progress_percent = ctk.CTkLabel(inner, text="0%", font=self.mgc.FONTS['title'], text_color=self.colors['blue'])
        self.progress_percent.pack()
        
        self.progress_label = ctk.CTkLabel(inner, text="Esperando inicio...", font=self.mgc.FONTS['small'], text_color=self.colors['text_secondary'])
        self.progress_label.pack()

        self.time_label = ctk.CTkLabel(inner, text="", font=self.mgc.FONTS['tiny'], text_color=self.colors['text_secondary'])
        self.time_label.pack(pady=(5, 0))
        
    def create_left_panel(self, parent):
        card_outer, inner = self.mgc.create_card(parent, "⚙️ CONFIGURACIÓN Y HERRAMIENTAS")
        card_outer.grid(row=0, column=0, sticky='nsew', padx=(0, 10))
        
        # Stats resumidos
        stats_frame = ctk.CTkFrame(inner, fg_color=self.colors['bg_primary'], corner_radius=8)
        stats_frame.pack(fill='x', pady=(0, 15), padx=5)
        self.stats_label = ctk.CTkLabel(stats_frame, text="👥 0 Emp. | 💰 $0.00", font=self.mgc.FONTS['heading'], text_color=self.colors['green'])
        ...
        ctk.CTkLabel(row_opts, text="Fuente Recibos:", font=self.mgc.FONTS['small']).pack(side='left', padx=5)
        font_dropdown = ctk.CTkOptionMenu(row_opts, variable=self.receipt_font, values=self.font_options, 
                                        fg_color=self.colors['bg_primary'], height=28)
        font_dropdown.pack(side='right', fill='x', expand=True, padx=5)
        
        # Grid de Herramientas de Limpieza
        ctk.CTkLabel(inner, text="Acciones de Limpieza (Directo en Excel):", font=self.mgc.FONTS['small'], 
                    text_color=self.colors['text_secondary']).pack(anchor='w', pady=(10, 5), padx=5)
        ...
        self.mgc.create_button(inner, "🔥 BORRADO GENERAL OPTIMIZADO", self.run_general_cleanup, color='red').pack(fill='x', pady=10, padx=5)
        
    def create_log_card(self, parent):
        card_outer, inner = self.mgc.create_card(parent, "📝 LOG DE ACTIVIDAD")
        card_outer.grid(row=0, column=1, sticky='nsew')
        
        self.log_text = ctk.CTkTextbox(inner, font=('Consolas', 11), fg_color='#0a0f1d', 
    ...
    def create_action_buttons(self, parent):
        ...
        self.del_btn.pack(side='left', padx=6)    # ===== FUNCIONES DE LÓGICA (mantener todas las existentes) =====
    ...
    def process(self):
        ...
        try:
            ...
            employee_df, day_definitions = load_structured_data(
                self.input_file, config, data_start_row, header_row, holiday_row
            )
            ...
            for index, employee_row in employee_df.iterrows():
                ...
                if pd.isna(raw_row_idx):
                    current_excel_row = data_start_row + index
                else:
                    try:
            ...
            total_amount = sum(r['Total Quincena'] for r in payroll_results)
